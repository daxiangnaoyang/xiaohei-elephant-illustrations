...
> 把中文文章里的学习卡点、流程断点、工具选择和 AI 协作处境，变成一张张有段落信息密度、真实物品关系和物理动作的“小黑象”视觉摘要。
>
> 默认正文配图：2.0 真实物品场景图 | 3:4 小红书手绘信息图（明确指定时） | 1.0 白底手绘解释图（明确指定时） | 21:9 Cover | 9:16 扇面视频封面 | Codex / Claude / Hermes Skill
...
它不是通用插画 prompt，也不是 PPT 信息图模板。它的核心目标是：先读完整段落，再把段落的主旨、处境、机制、结果和边界编译成一张有记忆点的视觉摘要。正文默认使用 2.0 真实物品小现场；只有明确指定 1.0 时才使用白底手绘解释图。
...
```text
小黑象 + 段落信息节点 + 真实物品/低科技物理隐喻 + 物理动作 + 短中文批注 + 留白
```
...
- 想要任意品牌广告 KV、精致扁平插画或产品截图的人（2.0 Cover 只承载具体主题隐喻，不是品牌广告）
...
- 16:9 横版真实物品小现场（2.0）
- 一篇文章的 4-6 张段落级 shot list，短文 1-3 张，长教程最多 8 张
- 每张图的插入位置、主题、核心意思、构图模式、小黑象动作、关键物件和中文短标签
- 按段落复杂度选择的 D1 / D2 / D3 信息密度
- 单张概念图 prompt
- 最终 PNG 图片
- 21:9 公众号 Cover 或 9:16 视频封面（按明确模式单独生成）
- 3:4 小红书手绘信息图卡片（明确提出“小黑象 + 小红书/3:4”时生成）
- 项目复盘 / 个人经历用的小黑象彩蛋长卷
...
- 16:9 横版、纯白或接近纯白摄影棚背景，真实物品有自然光影，镜头保持后退并留出呼吸区。
- 先读完整段落，再把输入/处境、机制/动作、结果/边界落实为真实物件、状态、空间关系或短标签。
- D1 只用于单一判断；D2 是解释型段落默认档；D3 用于多步骤、多条件、多角色、对比、分支、反馈或明确边界。每个物件必须有段落语义职责。
- 小黑象必须是短象鼻、小耳朵、白点眼、四足短圆桩腿的轻巧小象，并参与核心物理动作；信息增加时扩展真实物品现场，不放大角色。
- 画面轻、怪、清爽，但不做商品摄影、物品堆、卡片网格、PPT、UI 或复杂信息图。

### 1.0 明确指定时

- 16:9 纯白背景、黑色手绘线稿、低科技物理隐喻和少量中文短批注。
- 1.0 更适合拆观点、拆流程和拆方法；普通“小黑象配图”不再默认进入 1.0。

### 3:4 小红书手绘信息图分支

- 只在同时明确“小黑象 + 小红书 / 3:4 / 信息图”时启用；它是 2.0 的独立模式，不是 1.0 白底手绘解释图。
- 使用暖奶油纸面、手绘教育信息图、3-6 个段落信息节点和一条主关系线；保留扇面母版的平面泼墨、气泡、简笔图标、朱红印记和轻微放射式排布。
- “扇面”只指内容设计语言，不生成实体折扇；每张卡片独立生图、逐字白名单验收，不生成多页拼图或后期文字层。
...
```text
Use $xiaohei-elephant-scenes 先不要生图。
请分析下面这篇文章哪里适合做“小黑象 2.0 + 真实物品小现场”的正文配图。
输出 5 张左右的 shot list。

每张图写清楚：插入位置、原始段落、段落主旨、处境/输入、独立信息节点、D1/D2/D3 密度、真实物品及语义职责、小黑象动作、中文短标签、3 秒读懂句和风险/避坑。
...
```text
Use $xiaohei-elephant-scenes 把下面这篇文章生成 4 张小黑象 2.0 正文配图。
要求：16:9 横版、纯白摄影棚、先按完整段落选择 D1/D2/D3，再用真实物品和一条物理动作链表达输入 -> 机制 -> 结果；小黑象保持很小、轻巧并参与动作。
...
```text
Use $xiaohei-elephant-scenes 把下面这一段做成一张 3:4 小红书手绘信息图。
要求：保留扇面内容的暖纸面、中央单一泼墨、手绘气泡、简笔图标和朱红印记，但不要生成实体折扇；先理解完整段落，画出 3-6 个信息节点和它们的关系；小黑象只占画面 3%-5%，作为极小品牌锚点；所有文字逐字列入白名单并在同一次生图中融入。
...
```text
Use $xiaohei-elephant-scenes 为“AI 输出必须验收，不能直接发布”生成一张小黑象 2.0 正文配图。
请先理解完整段落；如果只有这一句话，标记缺失信息为“未获取”，再用一个真实物品、一个辅助状态和小黑象的物理动作表达验收关系，不要做成 PPT 流程图。
```
...
正文图先完成 shot record 的段落语义/密度和 3 秒读图检查，再由 Codex 当前 GPT 系列运行时完成一次轻量视觉门。3:4 小红书卡片、Cover、9:16 和长卷仍走各自的全量 QA。下面的脚本是 Hermes/智谱外部通道适配器，不是 Codex 默认视觉模型。
...
```text
Use $xiaohei-elephant-scenes 帮我编辑这张 2.0 真实物品场景图。
...
1. 读取文章、项目复盘、个人经历或主题
2. 按完整段落提炼主旨、段落类型、读者处境、独立信息节点、核心冲突和结果/边界
3. 根据段落复杂度选择 D1 / D2 / D3；解释型段落默认 D2，机制丰富的段落使用 D3
4. 将每个核心节点映射为真实物件、状态、空间位置、物理动作、关系线或短标签，再选择主物品和辅助物件
5. 让小黑象承担推、拉、挡、托、修、检查、盖章等核心动作
6. 生成 16:9 真实物品小现场，或生成 5-8 节点彩蛋长卷
7. 多图先生成第 1 张并通过 QA，冻结唯一角色母版；之后 02/03/04 等独立正文 job 默认最多 3 个并行，全部只引用同一母版
8. 标准正文图运行 `scripts/qa_image.py` 一次检查 IP、尺度和文字白名单；同时保留段落语义/密度与 3 秒读图门
9. Cover / 9:16 / 长卷继续执行对应全量 QA；Cover 候选最多 8 版，冲突按安全区/强制文案 > 可读性/内容 > 装饰审美裁决

公众号文章建议工作流：

```text
05-final-draft confirmed
-> use xiaohei-elephant-scenes by default
-> create 06-illustration-plan.md
-> generate shot 01, pass QA, freeze one master
-> fan out remaining independent body jobs (max 3 workers)
-> run body quick QA or Cover full QA
