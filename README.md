...
- 纯白或接近纯白摄影棚背景
- 先读完整段落并选择 D1 / D2 / D3 信息密度：D1 只用于单一判断，D2 是解释型段落默认档，D3 用于多步骤、多条件、多角色、对比、分支、反馈或明确边界的段落
- 每个段落节点必须落实为真实物件、状态、空间位置、物理动作、关系线或短标签；D2 通常使用 1 主 + 2-3 辅助，D3 可使用 1 主/紧凑主组 + 3-5 辅助物件或状态单元
- 小黑象和真实物品发生推、拉、挡、托、盖章、检查、修补等物理动作
- 真实物品有自然光影，但不能像商品摄影或素材堆
- 信息密度来自段落节点覆盖和物件关系，不来自无语义道具、长文字、卡片网格或 PPT 信息图
...
7. 按 QA 检查：段落节点覆盖、小黑象形体、真实物品、物理动作、留白、短标签、非旧小黑
