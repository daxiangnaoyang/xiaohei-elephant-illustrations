...
## 3:4 小红书手绘信息图视觉分支（XHS Handdrawn Infographic Variant）

本分支只用于“**小黑象 + 小红书 / 3:4 / 图文卡片 / 手绘信息图**”。它不是标准 16:9 真实物品图，不是 1.0 白底手绘解释图，也不是 9:16 视频封面。它把完整段落编译成一张单页教育信息图，同时保留扇面母版的平面内容语言。

```text
生成一张 3:4 竖版小红书单页内容卡。只生成一张独立卡片，不生成多页拼图、联系表、九宫格或轮播图合成。

主题：{{TOPIC}}
主标题（逐字准确显示）：{{TITLE_TEXT}}
标题重点词：{{TITLE_EMPHASIS}}
段落核心判断：{{PARAGRAPH_CORE}}
段落原始内容：{{SOURCE_PARAGRAPH}}
信息结构：{{LAYOUT}}
可见信息节点（3-6 个，每个写清语义职责和画面承载方式）：{{KEY_NODES}}
节点关系线：{{RELATIONSHIP}}
批注气泡（2-4 条短批注）：{{CALLOUTS}}
简笔图标（2-4 个，与节点一一对应）：{{ICON_SET}}
朱红印记（1-2 个，内容节奏点）：{{RED_MARKS}}
底部金句（逐字准确显示）：{{BOTTOM_QUOTE}}
主题强调色（只选一个）：{{ACCENT_COLOR}}
扇面风格母版（`STYLE_REFERENCE_ONLY`）：{{STYLE_REFERENCE}}
品牌锚点模式（只能选一个）：{{BRAND_ANCHOR_MODE}}
品牌锚点与版面的关系：{{BRAND_ANCHOR_RELATION}}
全部允许文字白名单：{{ALLOWED_TEXT}}
本次额外禁止项：{{AVOID}}

视觉风格：
Hand-drawn educational infographic on warm cream paper texture, 3:4 vertical Xiaohongshu content card. Use clean black hand-drawn lines with slight hand-drawn wobble, restrained editorial composition, generous whitespace, and visual hierarchy that can be read at phone-thumbnail size. No photographic realism, no product photography, no 3D render.

纸面与扇面语言：
- 背景是暖奶油纸张肌理 #F5F0E8，主文字和线条为墨黑，辅助批注为暖灰 #6B6B6B。
- 使用浅蓝 #A8D8EA、薄荷绿 #B5E5CF、薰衣草紫 #D5C6E0、浅桃 #F4C7AB 作为少量信息区色块；只保留一个主强调色，优先珊瑚红 #E8655A。
- 保留扇面母版的平面设计语言：轻微不规则纸面边缘、蓝绿低饱和边缘泼墨、中央单一主题刷痕、粗重略不规则中文手写字、轻微放射式排布、批注气泡、简笔图标和朱红印记。
- 扇面只是内容设计语言，不是实体载体。只生成一块平面内容画布；禁止实体折扇、扇片、扇骨、扇柄、折痕、手持关系、普通扇子图标和可辨识的扇形实体轮廓。

信息图结构：
- 按 {{LAYOUT}} 组织一条主视觉语法：流程/因果/对比/问题到解决/组成/层级/反馈，只选一种，不把物件或卡片横向排队。
- {{KEY_NODES}} 必须全部在画面中可见。每个节点使用图形、状态、位置、关系线、气泡或短标签承载，不能只靠文字点名。
- 画面必须同时表达“处境/问题 -> 机制/判断 -> 结果/边界”中的至少 3 个节点；不要把完整段落只摘成主题名词。
- 允许 2-4 个手绘区域、圆角色块、虚线框或气泡承载信息，但不能变成 UI、仪表盘、复杂流程图、PPT 卡片墙或密集节点墙。
- 图形优先，文字只做标题、短标签、批注和金句；辅助文字每条 2-6 个汉字，不生成大段正文。

3:4 手机构图与安全区：
- 顶部 0%-8% 保持低信息密度；标题放在 8%-22% 内容区，最多两行，作为第一视觉中心。
- 中央 22%-82% 放置 {{KEY_NODES}}、中央刷痕、气泡和简笔图标，保持清晰的阅读路径与充足间距。
- 底部 82%-88% 放置 {{BOTTOM_QUOTE}}，只保留一句，字号明显小于标题；底部 88%-100% 留给平台安全区，不放关键文字、品牌锚点或关键关系线。
- 缩小到手机缩略图后，3 秒内仍能读出主标题、一个关系线索和品牌锚点；不要让色块、印章或装饰遮住标题和节点。

小黑象 2.0 品牌锚点：
- 如果 {{BRAND_ANCHOR_MODE}} = 小黑象：加入一只极小的平面黑色小象，哑光黑实心剪影，短象鼻、小圆耳、白点眼、四足短圆桩腿、低矮轻巧身体；高度只有画面高 3%-5%，不成为吉祥物主角。
- 小黑象只能用短象鼻指向标题重点词、牵住关系线、压住下划线或盖住确认印记；不添加人类手臂和手掌，不做二足站姿。
- 如果 {{BRAND_ANCHOR_MODE}} = 官方大象 logo：只使用用户/项目提供的可核验官方资产，作为小角标或内容印记，尺寸克制、颜色准确；不得凭空编造 logo 或宣传口号。
- 两种品牌模式不能同时出现；品牌锚点缺失、漂浮、错误或抢主标题都判失败。

字体与文字：
- 主标题逐字显示“{{TITLE_TEXT}}”，使用粗重、友好、略不规则的中文手写字，参考 ZCOOL KuaiLe 的圆润手写气质；不得使用标准科技黑体、UI 字体、白字发光或海报广告字体。
- {{TITLE_EMPHASIS}} 使用 {{ACCENT_COLOR}} 的刷色强调；辅助批注使用墨黑或暖灰，保持短小、贴近对应节点。
- 底部金句逐字显示“{{BOTTOM_QUOTE}}”，只出现一次，融入纸面和手绘笔触。
- 图中只允许出现 {{ALLOWED_TEXT}}；禁止乱码、伪文字、额外英文、随机品牌名、水印、平台 UI、播放按钮、头像、点赞评论图标、时间戳和可读 HUD 小字。

ONE-PASS INTEGRATED COMPOSITION LOCK:
- 在同一次 image_gen 中同时生成暖纸面、扇面语言、主标题、中央刷痕、信息节点、关系线、气泡、图标、朱红印记、底部金句和小黑象品牌锚点。
- 可以使用一张登记的扇面母版作为 `STYLE_REFERENCE_ONLY`，只提取纸面、墨迹、版式和手绘语言；不得复制母版原题字、原印章文字、原气泡文字或原图解内容，也不得把母版当底图、文字层、mask 或合成素材。
- 可使用本次标题的 ZCOOL KuaiLe 排版参考图作为 `TYPOGRAPHY_REFERENCE_ONLY`，但它不是文字层；禁止本地叠字、透明文字 PNG、标题卡、base/overlay/composite/merged 中间产物和后期修字。

避免：
深色背景、灰黑底、深褐底、渐变、彩色晕染背景、实体折扇、扇片、扇骨、扇柄、折痕、手持摄影、扇子图标、PPT 信息图、UI、仪表盘、复杂流程图、密集卡片墙、随机贴纸、气泡堆叠、红色公章、错误或虚构 logo、品牌广告 KV、长段文字、标题超过两行、平台 UI、小黑人、人形腿、二足站姿、火柴腿、鞋子脚掌、厚重圆胖小象、3D 玩偶象、角色过大、只画主题名词而不表达段落机制和结果。
```

### 3:4 小红书模板变量填充顺序

1. 先读完整文章段落，填写 `{{SOURCE_PARAGRAPH}}`、`{{PARAGRAPH_CORE}}` 和 `{{KEY_NODES}}`；至少保留处境/问题、机制/判断、结果/边界中的 3 个节点。
2. 从节点关系选择一个 `{{LAYOUT}}` 和一条 `{{RELATIONSHIP}}`；不要先挑装饰，再倒推内容。
3. 把标题压缩为一个核心判断，建议 4-10 个汉字；再写一句短金句，全部加入 `{{ALLOWED_TEXT}}`。
4. 从文稿中选 2-4 条 `{{CALLOUTS}}`、2-4 个 `{{ICON_SET}}` 和 1-2 个 `{{RED_MARKS}}`；每个元素必须对应节点，不能补造数据、品牌、案例或机制。
5. `{{BRAND_ANCHOR_MODE}}` 默认填“小黑象”；只有用户明确选官方大象 logo 且提供可核验资产时才改填官方 logo。
