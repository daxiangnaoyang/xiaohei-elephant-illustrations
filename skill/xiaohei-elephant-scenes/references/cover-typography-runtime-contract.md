...
7. **验收不可降级**：Codex 默认使用当前 GPT 系列运行时做视觉理解；Hermes/智谱外部通道才使用 GLM 适配器。任一视觉 QA 工具返回 400、429、余额不足、不可用、超时或任何解析错误时，`visual_qa` 只能写 `BLOCKED` / `未验证`，不得写 `PASS`，不得把候选图当正式封面发送。没有可追溯的目视证据和回执，同样阻断交付。
